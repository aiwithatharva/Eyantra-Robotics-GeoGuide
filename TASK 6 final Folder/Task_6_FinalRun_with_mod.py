...
def read_event_dict():
    ...
    with open('Event_output.csv') as csv_file:
        ...
        for location, event in event_dict.items():    
            if event in event_priority:
                if event_priority[event]['position'] is None:
                    event_priority[event]['position'] = [location]
                else:
                    event_priority[event]['position'].append(location)
...
def get_position(event):
    positions = event_priority[event]['position']
...
def reconstruct_combined_path(event_priority, your_dictionary, next_vertex):
    ...
    for event, info in sorted(event_priority.items(), key=lambda x: x[1]['priority']):
        ...
        if positions :
            ...
            for j,position in enumerate(positions):
                ...
                end = next(key for key, value in your_dictionary.items() if value == position)
