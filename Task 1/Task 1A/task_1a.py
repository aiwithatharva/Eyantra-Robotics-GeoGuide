...
# Team ID:			[ GG_3303 ]
# Author List:		[ Atharva Satish Attarde, Ashutosh Anil Dongre, Nachiket Ganesh Apte, Prachit Suresh Deshinge ]
# Filename:			task_1a.py
...
def data_preprocessing(task_1a_dataframe):
	...
	mylist = ['Education', 'City', 'Gender', 'EverBenched', 'PaymentTier']
	encoded_dataframe = task_1a_dataframe
	for i in mylist:
		encoded_dataframe[i] = label_encoder.fit_transform(task_1a_dataframe[i])
	##########################################################
...
def identify_features_and_targets(encoded_dataframe):
	...
	#################	ADD YOUR CODE HERE	##################
	encoded_features = encoded_dataframe.drop('LeaveOrNot', axis=1)
	scalar = MinMaxScaler()
	scalar.fit(encoded_features)
	df = pandas.DataFrame(encoded_features)
	scaled_data = scalar.fit_transform(df)
	encoded_features = pandas.DataFrame(scaled_data, columns=df.columns)
	target = encoded_dataframe['LeaveOrNot']
	features_and_targets = [encoded_features, target]
	##########################################################
...
def load_as_tensors(features_and_targets):
	...
	feature_tensor_list = [torch.tensor(features_df.values.tolist(), dtype=torch.float32)]
	target_tensor_list = [torch.tensor(target_df.values.tolist(), dtype=torch.float32)]
	X_train, X_test, y_train, y_test = train_test_split(feature_tensor_list[0], target_tensor_list[0], test_size=0.2, random_state=42)
...
class Salary_Predictor(nn.Module):
	...
	def forward(self, x):
		...
		if model.training == True:
			...
			predicted_output = torch.round(x)
		###################################
...
def validation_function(trained_model, tensors_and_iterable_training_data):
	...
	'''	
	#################	ADD YOUR CODE HERE	##################
	X_test_tensor, _, y_test_tensor, _, test_loader = tensors_and_iterable_training_data
