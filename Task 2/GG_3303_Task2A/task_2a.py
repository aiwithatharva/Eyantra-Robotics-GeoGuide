...
def detect_ArUco_details(image):
    ...
    * ArUco_corners = 
    ...
    for i in range(len(ids)):
        center1 = (corners[i][0][0] + corners[i][0][2]) / 2.0
        center2 = (corners[i][0][1] + corners[i][0][3]) / 2.0
        ...
        average_top = (corners[i][0][0] + corners[i][0][1]) / 2.0
        average_bot = (corners[i][0][2] + corners[i][0][3]) / 2.0
        final_for_slope = average_top - average_bot
        angle = np.arctan2(final_for_slope[0], final_for_slope[1]) * 180 / np.pi
        angle = int(angle.round() - 180)
        if angle > 0 and angle > 180:
            angle -= 360
        elif angle < 0 and angle < -180:
            angle += 360
        finallist = [list(center), angle]
        ArUco_details_dict[int(ids[i])] = finallist
        ArUco_corners[int(ids[i])] = corners[i][0].astype(int)
    ##################################################
